      ...
      <div className="d-flex justify-content-between align-items-end">
        <div>
          <h5 className="mb-0 text-standard">
            <b style={{ color }}>{item.price} руб.</b>
          </h5>
          ...
          {onOrder ? (
            ...
            >
              {!item.quantity ? "Добавить в корзину" : `Добавить еще 1 (${item.quantity})`}
            </Button>
          ) : null}
        </div>

        {!item.is_available ? (<small className="text-secondary">Не доступно</small>) : null}
